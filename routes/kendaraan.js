...
router.get("/", function (req, res) {
  connection.query(
    " SELECT no_pol, kendaraan.nama_kendaraan, transmisi.nama_transmisi, kendaraan.gambar_kendaraan " +
      " FROM kendaraan JOIN transmisi " +
      " ON kendaraan.id_transmisi=transmisi.id_transmisi order by kendaraan.no_pol desc",
    function (err, rows) {
      if (err) {
...
router.get("/(:id)", function (req, res) {
  ...
  connection.query(
    `SELECT no_pol, kendaraan.nama_kendaraan, transmisi.nama_transmisi, kendaraan.gambar_kendaraan FROM kendaraan JOIN transmisi ON kendaraan.id_transmisi=transmisi.id_transmisi where no_pol='${id}'`,
    function (err, rows) {
      if (err) {
        return res.status(500).json({
          status: false,
  ...
  (req, res) => {
    ...
    connection.query(
      ...
      function (err, rows) {
        ...
        connection.query(
          `update kendaraan set ? where no_pol='${id}'`,
          data,
          function (err, rows) {
            if (err) {
              return res.status(500).json({
